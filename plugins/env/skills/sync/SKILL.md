---
name: sync
description: mccm.json 기반으로 마켓플레이스/플러그인/MCP/hooks/settings를 동기화한다. "/sync", "환경 동기화", "플러그인 동기화", "sync env" 등의 요청에 사용한다.
...
- 선택지 제시: **(1) 취소** (기존 유지) / **(2) 대치** (mccm.json 값으로 교체) / **(3) 추가** (둘 다 유지, 객체인 경우 병합)
- 사용자 선택에 따라 적용
